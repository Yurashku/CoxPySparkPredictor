...
- `examples/tutorial.ipynb` — Jupyter-ноутбук с полным циклом обучения, немедленным сохранением артефактов в CSV и перезапуском модели из этого файла.

### Что делает `tutorial.ipynb`

1. Генерирует синтетический датасет и обучает отдельные Cox-модели по типам.
2. Сразу после обучения продлевает baseline, сохраняет артефакты и конфиг в один CSV и показывает первые строки файла.
3. Создаёт свежий экземпляр модели через `SparkCoxPHByType.load` и через `from_config`, демонстрируя, что метаданные восстанавливаются из CSV.
4. Выполняет инференс на тренировочном датасете: сначала быстрый прогноз на фиксированный горизонт, затем предсказывает хвостовую выживаемость только для объектов без события (`event == 0`).
5. Все ключевые этапы сопровождаются `show()` и печатью промежуточных результатов для наглядности.
...
Ключевые классы и функции снабжены русскоязычными докстрингами и комментариями «для чего», чтобы быстрее понимать назначение шагов: от продления baseline и расчёта доверительных интервалов до политики семплирования и broadcast артефактов для инференса.

## Структура кода

- `model.py` — весь жизненный цикл `SparkCoxPHByType`: обучение, продление baseline, сохранение/загрузка, инференс.
- `udfs.py` — конструкторы векторизованных pandas UDF для предсказаний.
- `utils.py` — единая точка для вспомогательных функций: deterministic seed, сериализация в JSON, проверка и приведение колонок, а также `cap_sample_by_key`. Ранее эти утилиты были распределены по нескольким модулям, теперь они собраны вместе, чтобы уменьшить избыточную модульность и упростить навигацию.
